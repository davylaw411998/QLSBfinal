...
public class DaoSan {
    ...
    public void getDatSan(String user, String masan, String masuat, Date ngay) {            //static
        //ArrayList<SuatDat> list = new ArrayList<>();
        ...
        try {
            ...
            Connection connection = ConnectDB.getConnection(); // mở kết nối cơ sở dữ liệu
            Statement statement = connection.createStatement();// cho phép thực hiện truy vấn sql trên java
            String sql = "sp_insertDatSan '" + masan + "','" + masuat + "','" + ngay + "','" + user + "'";//"sp_TimSanTrong '"+loaisan+"','"+ngay+"','"+giodat+"'";
            int rs = statement.executeUpdate(sql);        // thực hiện truy vấn theo câu truy vấn truyền vào bằng hàm executeQuery của class statement
    ...
    public ArrayList<ThongTinDatSan> getShowThongTinSan() {            //static
        ArrayList<ThongTinDatSan> list = new ArrayList<>();
        try {
            ...
            ThongTinDatSan b;
            while (rs.next()) {
                b = new ThongTinDatSan(rs.getInt("MaDatSan"), rs.getInt("MaKH"), rs.getString("MaSan"), rs.getString("MaSuat"), rs.getDate("Ngay"));
                list.add(b);
            }
...
//                 d1.getDatSan("khachhang1",sd.getMaSan(), sd.getMaSuat(), sd.getNgay());
//             }   
